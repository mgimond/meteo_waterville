...
dat.smooth <- loess(temp ~ year + day, dat.lowess.wrap, span = 0.1, degree  = 2,
                    family = "symmetric")
smooth.df <- data.frame(dat.smooth$x, dat.smooth$fitted) %>% 
  filter(day > 0 & day < 366)
  
  ggplot(smooth.df) + aes( day , year, fill = `dat.smooth.fitted`,
                           z = `dat.smooth.fitted`) + 
  geom_tile() +
  scale_fill_gradient2(low = "blue", high = "red", mid = "white", 
                       midpoint = 32) +
  geom_contour(size = 0.5, color = "grey", breaks = seq(-0, 85, by = 5))
