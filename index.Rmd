...
## Sooth of normalized temperature data
...
dat.loess <- dat %>% 
  ...
  mutate(missing = as.numeric(is.na(Temp)),
         dec.year = decimal_date(Date_EST),
         year = round(dec.year),
         year.frac = dec.year - year)

dat.smooth <- loess(Temp ~ year + year.frac, dat.loess)
smooth.df <- data.frame(dat.smooth$x, dat.smooth$fitted)
  
  ggplot(smooth.df) + aes( year.frac,year, fill = `dat.smooth.fitted`) + 
  geom_tile() 
